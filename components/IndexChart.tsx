      ...
      {showLevels && (
        <div className={`border-t pt-2 space-y-1 ${isDark ? "border-zinc-700" : "border-zinc-100"}`}>
          <div className="flex justify-between gap-4">
            <span style={{ color: "#31572c" }} className="font-semibold">+2σ</span>
            <span style={{ color: "#31572c" }} className="font-semibold">{fmt(controlLines.sd2Upper)}</span>
          </div>
          <div className="flex justify-between gap-4">
            <span style={{ color: "#4f772d" }} className="font-semibold">+1σ</span>
            <span style={{ color: "#4f772d" }} className="font-semibold">{fmt(controlLines.sd1Upper)}</span>
          </div>
          <div className="flex justify-between gap-4">
...
export function IndexChart({ data, metric, controlLines, showControlLines, valueFormatter }: IndexChartProps) {
  ...
  const controlLevelStrip = showControlLines && controlLines ? [
    { label: "+2σ", value: controlLines.sd2Upper, color: "#31572c" },
    { label: "+1σ", value: controlLines.sd1Upper, color: "#4f772d" },
        ...
        {showControlLines && controlLines && (
          <>
            ...
            <ReferenceLine
              ...
              strokeDasharray="3 5"
              strokeWidth={1}
              label={{ value: "+1σ", position: "insideTopRight", fontSize: 10, fill: "#4f772d" }}
            />
            <ReferenceLine
            ...
            <ReferenceLine
              ...
              strokeDasharray="2 6"
              strokeWidth={1}
              label={{ value: "+2σ", position: "insideTopRight", fontSize: 10, fill: "#31572c" }}
            />
            <ReferenceLine
